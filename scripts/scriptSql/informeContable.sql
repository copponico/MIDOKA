...
JOIN
  (SELECT 'ACTUAL' AS ACTUAL, SUM(DEBE) AS 'COMPRA' FROM SALDO_PROOVEDOR JOIN(OPERACIONES) ON(opID=OPERACION_ASOCIADA) WHERE OPERACIONES.FECHA<='Lsup' AND OPERACIONES.FECHA>'Linf'xshe) t17 ON(t1.ACTUAL=t17.ACTUAL)
JOIN
